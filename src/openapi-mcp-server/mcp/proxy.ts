...
export class MCPProxy {
   ...
   *   headers are resolved from the environment — per-workspace NOTION_TOKEN_*
   *   first, then legacy OPENAPI_MCP_HEADERS / NOTION_TOKEN. Passing headers
   *   explicitly BYPASSES workspace discovery and configures a single
   *   'default' client; that is what the HTTP transport uses so one deployment
   *   can serve a different Notion integration per connection.
   */
  constructor(name: string, openApiSpec: OpenAPIV3.Document, headers?: Record<string, string>) {
    // Wider than upstream's `{ tools: {} }` on purpose: this fork implements
    // prompts and resources handlers too (see setupHandlers).
    ...
    if (headers && Object.keys(headers).length > 0) {
      // Explicit headers: one caller-supplied integration, no env discovery.
      const client = new HttpClient({ baseUrl, headers }, openApiSpec)
